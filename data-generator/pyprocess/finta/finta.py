#
# Downloaded from https://pypi.org/project/finta/
# Give credit!
#
...
class TA:
    ...
    def EMA(
        ...
        return pd.Series(
            ohlc[column].ewm(span=period, adjust=adjust, min_periods=min_periods).mean(),
    ...
    @classmethod
    @inputvalidator(input_="ohlcv")
    def VWAPN(cls, ohlcv: DataFrame, period: int = 14) -> Series:
        """
        [Tomar]
        VWAP for the past N candles.
        This acts as a fair indication of the most recent price traders are
        willing to pay for the stock.
        """

        return pd.Series(
            ((ohlcv["volume"] * cls.TP(ohlcv)).rolling(window=period).sum() /
                ohlcv["volume"].rolling(window=period).sum()),
            name="{0} period VWAPN".format(period)
        )
