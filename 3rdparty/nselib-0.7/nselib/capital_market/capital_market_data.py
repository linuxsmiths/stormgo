...
def index_data(index:str, from_date: str = None, to_date: str = None, period: str = None):
    ...
    while load_days > 0:
        ...
        #print("index=%s start_date=%s end_date=%s data_df=%s" % (index, start_date, end_date, data_df))
        from_date = from_date + dt.timedelta(365)
        load_days = (to_date - from_date).days
        nse_df = pd.concat([nse_df, data_df], ignore_index=True)
    return nse_df
...
#
# TOMAR
# Sample curl command to fetch data from NSE
#
# curl -H 'Connection: keep-alive' -H 'Cache-Control: max-age=0' -H 'DNT: 1' -H 'Upgrade-Insecure-Requests: 1' -H 'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/111.0.0.0 Safari/537.36' 'https://www.nseindia.com/api/historical/indicesHistory?indexType=NIFTY%2050&from=01-01-2024&to=31-12-2024'
#
