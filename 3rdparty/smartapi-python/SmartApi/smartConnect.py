...
class SmartConnect(object):
    ...
    try:
        # XXX
        # TOMAR: api.ipify.org sometimes hangs for a very long time.
        # Fastest response is seen from icanhazip.com, then we have others
        # which work better than api.ipify.org.
        # XXX
        #clientPublicIp= " " + get('https://api.ipify.org').text
        #clientPublicIp= " " + get('http://ipecho.net/plain').text
        #clientPublicIp= " " + get('http://ipinfo.io/ip').text
        #clientPublicIp= " " + get('http://ifconfig.me').text
        clientPublicIp= " " + get('http://icanhazip.com').text
    ...
    #   
    # TOMAR
    # Export postRequest() for making some calls which are not provided by the
    # module.
    #   
    def postRequest(self, route, params=None):
        """Alias for sending a POST request."""
        return self._postRequest(route, params)
    ...
    def generateSession(self,clientCode,password,totp):
        ...
        if loginResultObject['status']==True:
            ...
            #
            # TOMAR
            # Sometimes I've seen this fail with the following json:
            # {
            ...
            # }
            #
            # Accessing user['data']['clientCode'] and other members will
            # result in confusing "'NoneType' object is not subscriptable"
            # error. Check and fail the call returning None.
            #
            if user['status'] != True:
                logger.error(f"getProfile failed: {user}")
                return None
